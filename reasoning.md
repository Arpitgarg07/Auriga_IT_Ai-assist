...
`getBestStreak` sorts all recorded completion keys and scans them in order. Consecutive calendar keys extend a run; a gap resets it. Since the weekday schedule excludes weekends, the scan's scheduled-day handling allows a Friday-to-Monday completion to remain consecutive. No history returns zero.
...
The available executable checks are the repository's `npm run lint` and `npm run build`. The streak utilities are kept pure and reusable so daily, weekday, weekend-gap, missing-day, today-incomplete, and empty-history examples can be exercised directly in a future unit-test suite. A dedicated test runner was not added because it was not already part of the small dependency set.

## Trade-offs

A single component keeps the assessment implementation fast, but it means `App.jsx` is denser than a larger production codebase would be. `localStorage` is reliable for a single offline browser but has no synchronization or recovery across devices. The challenge start date is persisted but not editable in the current UI. The typography uses a Google Fonts stylesheet for visual polish; all behavior remains local and the browser falls back to the declared font family when the font cannot load.
